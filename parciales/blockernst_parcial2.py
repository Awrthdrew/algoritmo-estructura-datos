...
def bubbleSort(lista):
    for i in range(len(lista)-1):
        for j in range(len(lista)-i-1):
            if lista[j].promedio_goles_por_partido() > lista[j+1].promedio_goles_por_partido(): #EL ORDEN ES DESCENDENTE SI PONES <
                aux = lista[j]
                lista[j] = lista[j+1]
                lista[j+1] = aux 
...
def insertionSort(lista):
    for i in range(1, len(lista)):
        key = lista[i]
        j = i - 1
        while j >= 0 and lista[j].promedio_goles_por_partido() > key.promedio_goles_por_partido(): #EL ORDEN ES DESCENDENTE SI PONES <
            lista[j+1] = lista[j]
            j -= 1
        lista[j+1] = key
...
def partition(array, low, high):
  ...
  for j in range(low, high):
    if array[j].promedio_goles_por_partido() >= pivot:                                      #EL ORDEN ES DESCENDENTE SI PONES >=
...
def quickSort(array, low, high): #HIGH ES EL LARGO DEL ARRAY y LOW VALE 0 QUE ES EL LARGO DEL ARRAY
  if low < high:
    pi = partition(array, low, high)
    # ordenar a la izq del punto particionado
    quickSort(array, low, pi - 1)
    #ORDENA DESDE EL PIVOT MENOS 1 HACIA EL INICIO DEL ARRAY. OSEA EL SUB ARRAY IZQUIERDO
...
def mergeSort(array):
    if len(array) > 1:
        ...
        while i < len(L) and j < len(R):
            if L[i].promedio_goles_por_partido() < R[j].promedio_goles_por_partido(): #EL ORDEN ES DESCENDENTE SI PONES >
                array[k] = L[i]
                i += 1
            else:
                array[k] = R[j]
...
# PRINT DE LOS JUGADORES DESORDENADOS
